...
function checkingValues(values: string[], phrase: string): boolean {
  const regExpZPhrase = new RegExp(phrase, "i");
  let letters = /^.{0,2}$/;
  let isFind = false;

  if (letters.test(phrase)) {
    return false;
  }

  for (let i = 0; i < values.length; i++) {
    let checkedValue: string | object | [] = values[i];
    if (regExpZPhrase.test(checkedValue) && phrase != "") {
      isFind = true;
    } else if (Array.isArray(checkedValue)) {
      if (checkingValues(checkedValue, phrase)) {
        isFind = true;
      }
    } else if (typeof checkedValue === "object") {
      let checkedValueInObject: string[] = Object.values(checkedValue);
      if (checkingValues(checkedValueInObject, phrase)) {
        isFind = true;
      }
    }
  }
